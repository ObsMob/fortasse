...
from game_wasd import get_keypress, wasd_walk
from config import (
    GameResult,
    RevealType,
    PostGameAction,
...
def game_init(settings, term, board, restart):
    ellipsis = "."
    last_update = 0
    update_interval = 0.3
    ...
    while True:
        ...
        new_board.selected = new_board.get_tile_from_coords((new_board.depth, 1))
    ...
    game_result = game_start(settings, term, new_board)

    return game_result, new_board

def game_start(settings, term, board):
    render = board.board_render
    invalid = False
    ...
    while True:    
        ...
        if select_tile_not_action and not settings["WASD"]:
            board.selected = None
        ...
        if select_tile_not_action:
            cursor.print_default_board_option(settings)
            cursor.reset_line()
            
            if settings["WASD"]:
                key = get_keypress(term)
                
                if key == KeyPress.SELECT:
                    select_tile_not_action = False
                elif key == KeyPress.QUIT:
                    return GameResult.QUIT
                else:
                    if key != None:
                        board.selected = wasd_walk(board, key)
                        pos = board.selected.state_coords
                continue
                ...
                if pos is not None:
                    ...
                    if tile != None and tile.is_revealed:
                        ...
                        invalid = True
                        invalid_state = True
                    elif 1 <= pos[0] <= board.depth and 1 <= pos[1] <= board.depth:
                        board.selected = tile
                        select_tile_not_action = False
                        valid = True
                        valid_move = True
            ...
            if user_input == "B":
                ...
                reveal = board.selected.reveal_tile()
